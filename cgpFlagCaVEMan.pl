#!/home/ypan6/software/perl-5.36.3/bin/perl
...
sub getVCFToAddResultsOfFilters{
	...
	my $result = $flagger->runProcess($chr,$pos,$pos,$wt,$mut);
        if (!exists($flagger->_muts->{'tqs'}) || scalar(@{$flagger->_muts->{'tqs'}}) == 0) {
            warn "Warning: _muts->{'tqs'} is missing or empty for variant at $chr:$pos. Skipping variant.\n";
            $resFlags{'skipped'} = 1;  # Set a custom filter value for skipped variants
            return \%resFlags;  # Return empty result to skip the variant
        }
	...
	foreach my $flagName(@$flagList){
		...
		#Run this flag
		#print "Processing variant at $chr:$pos using flag: $flagName\n";  # DEBUG PRINT
		my $flagRes = runFlagger($flagger,$flagName,$flagId,$chr,$pos,$mut,$x,$vcf,$configParams,$isInUmVCF,$tabixList);
                #print "Finished processing $chr:$pos with flag: $flagName\n";  # DEBUG PRINT
