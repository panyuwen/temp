...
sub runProcess{
	...
	$self->{'nb'}->fetch($chr.':'.$start.'-'.$stop,\&_callbackMatchedNormFetch);
        if (!defined $muts || !exists $muts->{'tqs'} || !@{$muts->{'tqs'}}) {
            warn "Warning: _muts is undefined for variant at $chr:$start-$stop. Skipping variant.\n";
            return 0;  # Indicate that the variant should be skipped
        }
        return 1;
}
